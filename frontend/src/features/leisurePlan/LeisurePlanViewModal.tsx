...
import { X, Calendar, MapPin, Users, Plane, Hotel, Clock, Utensils, CheckCircle2 } from 'lucide-react';
import type { LeisurePlan } from './services/leisurePlanApi';
import StripePayButton from '../../components/StripePayButton';

interface LeisurePlanViewModalProps {
    isOpen: boolean;
    onClose: () => void;
    plan: LeisurePlan | null;
}

export default function LeisurePlanViewModal({ isOpen, onClose, plan }: LeisurePlanViewModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white w-full max-w-5xl max-h-[90vh] rounded-[32px] shadow-2xl flex flex-col overflow-hidden font-poppins animate-in fade-in zoom-in duration-300">
                ...
                <div className="px-8 py-6 flex items-center justify-between border-b border-gray-100 bg-[#1a2b6b]">
                    <div>
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${plan.status === 'Confirmed' ? 'bg-emerald-500 text-white' :
                                plan.status === 'Cancelled' ? 'bg-red-500 text-white' : 'bg-amber-500 text-white'
                                }`}>
                                {plan.status}
                            </span>
                        </div>
                        <p className="text-sm text-white/70 mt-1">Full Plan Overview & Itinerary</p>
                    </div>
                ...
                {/* Content */}
                <div className="flex-1 overflow-y-auto px-8 py-8 space-y-10 scrollbar-hide bg-gray-50/50">
                    ...
                    {/* Payment Error section removed as it's handled by StripePayButton */}

                    {/* Footer */}
                    <div className="pt-8 border-t border-gray-100 flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            <button className="px-6 py-3 bg-[#1a2b6b] text-white text-xs font-bold rounded-xl hover:opacity-90 transition-all shadow-lg shadow-blue-900/10">
                                Download Detailed PDF
                            </button>
                            <button className="px-6 py-3 bg-white border border-gray-100 text-[#1a2b6b] text-xs font-bold rounded-xl hover:bg-gray-50 transition-all">
                                Share Itinerary
                            </button>
                            {plan.status === 'Pending' && (
                                <StripePayButton
                                    ...
                                    label={`Book Now — £${plan.estimatedCost.toLocaleString()}`}
                                    wrapperClassName="shrink-0"
                                    className="!w-auto !py-3 !px-6 !rounded-xl !shadow-lg shadow-orange-200 !text-xs !bg-[#f2ae66]"
                                    hideBadge={true}
                                />
                            )}
                        </div>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                            Booking ID: <span className="text-[#1a2b6b]">SAVED-{plan.id}</span>
                        </p>
                    </div>
                </div>
            </div>
