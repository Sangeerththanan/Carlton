...
namespace Carlton.CustomerSelfService.Features.Flights.Services;
...
public class FlightService : IFlightService
{
    ...
    public async Task<FlightDto> CreateFlightAsync(CreateFlightDto createFlightDto)
    {
        ...
        var flight = new Flight
        {
            ...
            Airline = string.IsNullOrWhiteSpace(createFlightDto.Airline) ? "Unknown Airline" : createFlightDto.Airline,
            Stops = createFlightDto.Stops ?? 0,
            HasCheckInBaggage = createFlightDto.HasCheckInBaggage ?? false,
            HasHandLuggage = createFlightDto.HasHandLuggage ?? true
        };
    ...
    public async Task<FlightDto?> UpdateFlightAsync(int id, UpdateFlightDto updateFlightDto)
    {
        ...
        var flight = new Flight
        {
            ...
            Airline = string.IsNullOrWhiteSpace(updateFlightDto.Airline) ? "Unknown Airline" : updateFlightDto.Airline,
            Stops = updateFlightDto.Stops ?? 0,
            HasCheckInBaggage = updateFlightDto.HasCheckInBaggage ?? false,
            HasHandLuggage = updateFlightDto.HasHandLuggage ?? true
        };
    ...
    private static FlightDto MapToDto(Flight flight)
    {
        return new FlightDto
        {
            ...
            Stops = flight.Stops,
            HasCheckInBaggage = flight.HasCheckInBaggage,
            HasHandLuggage = flight.HasHandLuggage
        };
    }
